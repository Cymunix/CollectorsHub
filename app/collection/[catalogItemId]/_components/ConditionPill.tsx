...
function normalizeStorageUrl(raw: string | null): string | null {
  if (!raw) return null;
  const s = String(raw).trim();
  if (!s) return null;
  if (s.startsWith("http://") || s.startsWith("https://")) return s;

  const bucket = (process.env.NEXT_PUBLIC_CATALOG_MINIFIG_PHOTO_BUCKET as string | undefined) ?? "catalog";
  try {
...
export default function MinifigPanel({
  ...
  async function updateRow(rowId: string, patch: Partial<Pick<Row, "included" | "included_qty" | "notes">>) {
    ...
    return (
      <div className="rounded-xl bg-gray-50 border overflow-hidden">
        {rows.map((r) => {
          ...
          const img = normalizeStorageUrl(m?.image_url ?? null);
          const busy = !!savingIds[r.id];
          const qty = clampQty(r.included_qty);

          const go = () => router.push(`/collection/minifigs/${encodeURIComponent(r.minifig_id)}`);
          ...
          return (
            <div key={r.id} className="px-4 py-3 border-b last:border-b-0 bg-white">
              <div className="flex items-start gap-3">
                <button
                  type="button"
                  className="h-10 w-10 rounded-lg border bg-gray-50 overflow-hidden flex items-center justify-center shrink-0 hover:opacity-90"
                  onClick={go}
                  title="Open minifig in your collection"
                >
                  {img ? (
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center justify-between gap-3">
                    <div className="min-w-0">
                      <button
                        type="button"
                        className="text-sm font-semibold text-gray-900 truncate hover:underline underline-offset-4"
                        onClick={go}
                      >
                        {label}
                      </button>
  ...
  }, [err, loading, rows, readOnly, savingIds, router, load]);
