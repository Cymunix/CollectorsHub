...
function ComingSoonPanel({
  ...
  return (
    <div className="rounded-2xl border bg-white p-6 shadow-sm">
      ...
      <div className="mt-5 rounded-xl border border-dashed p-5 text-sm text-gray-500">
        Coming soon.
      </div>
    </div>
  );
...
export default function WishlistPage() {
  ...
  // Lookups for filters (same style as catalog page)
  ...
  const loadWishlist = async () => {
    ...
    try {
      // 1) Pull wishlist rows for this user
      ...
      // 4) Category names used to determine kind
      const catNameMap = new Map<string, string>();
      for (const c of categories) catNameMap.set(c.id, c.name);

      // 5) Detail tables for subtitle + dynamic filter matching
      ...
      // 6) Build cards (keep wishlist order)
  ...
  useEffect(() => {
    // Load metadata first (needed for subtitles + kind detection)
    loadMeta();
  ...
  useEffect(() => {
    if (!user?.userId) return;
    if (categories.length === 0) return; // wait for meta (kind detection uses category names)
    loadWishlist();
    // eslint-disable-next-line react-hooks/exhaustive-deps
              ...
              <div className="rounded-2xl border bg-white p-4 shadow-sm max-h-[calc(100vh-8rem)] overflow-y-auto">
                ...
                <div className="mt-3 space-y-3 text-xs">
                  ...
                  <div className="space-y-1">
                    ...
                    <select value={categoryId} onChange={(e) => setCategoryId(e.target.value)} className="w-full rounded-xl border bg-white px-3 py-2">
                  ...
                  <div className="space-y-1">
                    ...
                    <select value={franchiseId} onChange={(e) => setFranchiseId(e.target.value)} className="w-full rounded-xl border bg-white px-3 py-2">
                      <option value="">All</option>
                      {franchises.map((f) => (
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    <div className="space-y-1">
                      ...
                      <input value={minYear} onChange={(e) => setMinYear(e.target.value)} className="w-full rounded-xl border bg-white px-3 py-2" inputMode="numeric" />
                    </div>
                    <div className="space-y-1">
                      <label className="font-medium">Max Year</label>
                      <input value={maxYear} onChange={(e) => setMaxYear(e.target.value)} className="w-full rounded-xl border bg-white px-3 py-2" inputMode="numeric" />
                    </div>
                  </div>
                  ...
                  {selectedKind === "building_blocks" && (
                    <div className="pt-2 border-t space-y-3">
                      ...
                      <div className="space-y-1">
                        ...
                        <select value={bbThemeId} onChange={(e) => setBbThemeId(e.target.value)} className="w-full rounded-xl border bg-white px-3 py-2">
                          <option value="">All</option>
                          {bbThemes.map((t) => (
                      ...
                      <div className="space-y-1">
                        ...
                        <select value={bbSubthemeId} onChange={(e) => setBbSubthemeId(e.target.value)} className="w-full rounded-xl border bg-white px-3 py-2">
                          <option value="">All</option>
                          {bbSubthemes.map((t) => (
                            <option key={t.id} value={t.id}>
                              {t.name}
                  ...
                  {selectedKind === "toy" && (
                    <div className="pt-2 border-t space-y-3">
                      ...
                      <div className="space-y-1">
                        ...
                        <select value={toyManufacturerId} onChange={(e) => setToyManufacturerId(e.target.value)} className="w-full rounded-xl border bg-white px-3 py-2">
                          <option value="">All</option>
                  ...
                  {(selectedKind === "trading_card" || selectedKind === "sports_card") && (
                    <div className="pt-2 border-t space-y-3">
                      ...
                      <div className="space-y-1">
                        <label className="font-medium">Manufacturer</label>
                        <select value={cardManufacturerId} onChange={(e) => setCardManufacturerId(e.target.value)} className="w-full rounded-xl border bg-white px-3 py-2">
                          <option value="">All</option>
                      ...
                      <div className="space-y-1">
                        ...
                        <select value={cardTypeId} onChange={(e) => setCardTypeId(e.target.value)} className="w-full rounded-xl border bg-white px-3 py-2">
                          <option value="">All</option>
                          {cardTypes.map((t) => (
                            <option key={t.id} value={t.id}>
                              {t.name}
                  ...
                  {selectedKind === "music" && (
                    <div className="pt-2 border-t space-y-3">
                      ...
                      <div className="space-y-1">
                        ...
                        <select value={musicArtistId} onChange={(e) => setMusicArtistId(e.target.value)} className="w-full rounded-xl border bg-white px-3 py-2">
                          <option value="">All</option>
                          {musicArtists.map((a) => (
                  ...
                  {selectedKind === "gaming" && (
                    <div className="pt-2 border-t space-y-3">
                      ...
                      <div className="space-y-1">
                        ...
                        <select value={gamePlatformId} onChange={(e) => setGamePlatformId(e.target.value)} className="w-full rounded-xl border bg-white px-3 py-2">
                          <option value="">All</option>
                          {gamePlatforms.map((p) => (
                  ...
                  {selectedKind === "comic" && (
                    <div className="pt-2 border-t space-y-3">
                      ...
                      <div className="space-y-1">
                        ...
                        <select value={comicPublisherId} onChange={(e) => setComicPublisherId(e.target.value)} className="w-full rounded-xl border bg-white px-3 py-2">
                          <option value="">All</option>
                              ...
                              >
                                ...
                                <div className="p-3">
                                  ...
                                  <div className="mt-2 flex items-center justify-between">
                                    <span className="text-[10px] text-gray-400">{it.release_year ? it.release_year : ""}</span>
